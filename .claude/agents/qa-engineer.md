...
- `qa-verification` — the governing process for every verdict.
- `physics-invariants` — run the full battery on every env-physics verdict; the verdict must list which invariants ran on which scenarios.
- `validate-telemetry` — any producer/consumer deliverable you verify must include full-message validation against the LOCKED schema.
- `pr-merge-gate` — when asked whether a PR's gates have passed.
