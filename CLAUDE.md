...
- **CI tiers (D30):** four workflows. `ci-fast` (REQUIRED, every PR, `-m "not slow"`, <10 min). `ci-slow-env` is a **conditionally-required env gate** — it always runs as a single job (safe to mark required) but only does real work on env-path changes, running `-m "slow and not acceptance and not fullyear" -n 2` (bounded parallelism + `fullyear` excluded to avoid runner OOM). `ci-slow-acceptance` (path-filtered + nightly, `-m "acceptance"`, advisory). `ci-nightly` runs the full suite **serially** (no `-n`) and is **release-blocking**. Markers: `slow` (heavy compute, off the fast job), `acceptance` (real-install §9.5; also `slow`), `fullyear` (the 8760-step full-year parity; subset of `slow`, nightly-serial only). New heavy env/JAX tests MUST be marked `slow` (and `fullyear` if a genuine 8760-step rollout).
