...
- **CI tiers (D30):** the required PR check is the `fast` job (`pytest -m "not slow"` + frontend + conventions, <10 min). Heavy tests MUST be marked `@pytest.mark.slow` (compute: env battery, full-year parity, large JAX compile) or `@pytest.mark.acceptance` (real-install/venv §9.5); these run in conditional path-filtered `slow-env` / `slow-acceptance` jobs + nightly. CI is an early signal — QA's local full run remains the authoritative merge gate. When you add a test that compiles large JAX graphs, runs a full-year rollout, or shells out to an installer, mark it `slow`/`acceptance`.
