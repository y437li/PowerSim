...
- Fails (exit 1) iff any of `static-checks`/`backend-tests`/`frontend-tests` has
  `.result ∈ {failure, cancelled}`. `skipped` and `success` → pass.
...
- **(B)** **Canary demonstrates the gate BLOCKS** (per §7.2): broken-backend PR AND broken-frontend PR
  each show `checks` FAILURE + merge blocked; recorded with run URLs. **Hard gate — no merge of the
  quick-wins PR until B is shown.**
...
- **(F)** No `-m "not slow and not local"` test OOMs at `-n 2` on the 2 GB runner (empirically shown,
  #124 pattern). Report peak behavior / completion without SIGTERM.
...
- `# reviewer:` (to be added by backend-reviewer) — any additional canary scenarios.
- **Schema-conformance unit cases** (under `tests/shared/`, owned by each schema's contract):
  - finance-summary: a known-good producer fixture validates against #135 v1.1.0; a fixture with
    `equity_irr_pct` as MetricPercentiles (not scalar) → **rejected**; a non-NPV metric carrying
    `bootstrap_ci` → **rejected** (rule B); mismatched per-metric `confidence` at same q → **rejected**
    (rule A).
  - config-artifact: a known-good artifact validates against #133 v1.0.0; a `finance_overrides` key
    outside the frozen allow-set → **rejected**; a secret-shaped value → **rejected** (D32).
- **Aggregator-logic cases** (the `checks` exit-code rule), expressed as canary scenarios B1–B4 in §7.2.
