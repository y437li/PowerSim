...
| FIN-58 | **INV-DEG §3.6 cash half (calendar arm):** `build_cash_flow_series([yr,yr], econ=econ_lifecycle)` with `degradation_yuan=¥100k` (decoy) and `lifetime_years=1, replacement_cost_fraction=0.70` → CF[1]=−¥100k (EBITDA ¥600k − replacement ¥700k); degradation NOT subtracted (if wrong: −¥200k) | PR #114 §B; team-lead GO |
