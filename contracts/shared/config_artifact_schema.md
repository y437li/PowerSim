...
FinanceOverrides:        # a PARTIAL FinanceConfig — only the fields this config overrides
  <subset of FinanceConfig fields>   # financing-structure ONLY, EXACT FinanceConfig keys (FROZEN, finance-expert-ruled): { debt_toggle?, target_de_ratio?, credit_spread?, loan_term_years? }
```
...
- **`finance_overrides` MAY carry ONLY the FROZEN financing-structure allow-set, named by EXACT `FinanceConfig` keys (finance-expert-ruled 2026-06-14):** **`{ debt_toggle, target_de_ratio, credit_spread, loan_term_years }`** — the legitimate capital-structure knobs that may differ between compared configs (all-equity vs levered; 60% vs 70% gearing; wider debt spread; different tenor), D43. **`r_d_override` and `hurdle_rate_override` are EXCLUDED** (finance-expert): `r_d_override` is a test/script bypass of the principled LPR+`credit_spread` derivation — a real per-config cost of debt is expressed via `credit_spread` over the common base, so a per-config override would let a config hardcode a rate decoupled from the rate model (flattering-stub risk); `hurdle_rate_override` is excluded because the per-config hurdle **already varies correctly** via the Hamada relever (`β_L = β_U·(1+(1−tax)·D/E)`, and `target_de_ratio` IS per-config) → a levered config already gets a higher equity hurdle derived from the COMMON β_U/ERP/r_f, so a free hurdle override would **decouple the hurdle from capital structure (double-count) and muddy the ranking**. Any key not in the allow-set is **REJECTED** with: *"market-rate is common-scenario, not per-config; set it on `ComparisonSession.common_finance`."* (Default-deny: any future `FinanceConfig` field is common-only until explicitly added to this set by a superseding DECISION — finance-expert-owned.)
...
14. **# reviewer: finance_overrides scope-restriction (D42 apples-to-apples)** — `finance_overrides` may ONLY carry financing-structure fields named by their **EXACT `FinanceConfig` keys** — the FROZEN allow-set `{ debt_toggle, target_de_ratio, credit_spread, loan_term_years }` (finance-expert-ruled; `r_d_override` + `hurdle_rate_override` EXCLUDED). An override of any excluded/market-rate field — `r_d_override`, `hurdle_rate_override`, or any CAPM input (any CAPM input — `beta_unlevered`/`equity_risk_premium`/`country_risk_premium`/`cgb_curve`, `tax_rate`/`tax_toggle`, `horizon_years`, `valuation_date`, escalation, or `price_path`) → **reject** ("market-rate is common-scenario, not per-config; set it on `ComparisonSession.common_finance`"). Without this, two compared configs could silently use different discount rates → the D42 apples-to-apples guarantee breaks. (hand example: `finance_overrides={target_de_ratio:0.6}` → accept; `finance_overrides={equity_risk_premium:0.07}` → reject.)
