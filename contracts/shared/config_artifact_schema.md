...
FinanceOverrides:        # a PARTIAL FinanceConfig — only the fields this config overrides
  <subset of FinanceConfig fields>   # financing-structure ONLY, EXACT FinanceConfig keys: { debt_toggle?, target_de_ratio?, credit_spread?, loan_term_years? } (+ r_d_override?/hurdle_rate_override? pending finance-expert — §4 rules)
```
...
- **`finance_overrides` MAY carry ONLY financing-structure fields, named by their EXACT `FinanceConfig` field keys** (backend-reviewer — a default-deny validator checks override KEYS against this set, so it MUST use real keys, not conceptual names). **Confirmed-clear allow-set: `{ debt_toggle, target_de_ratio, credit_spread, loan_term_years }`** (the capital-structure knobs, D43 "a config may carry its own capital structure"). **PENDING finance-expert ruling (2 members):** `r_d_override` (documented as a *unit-test bypass* of the LPR+credit_spread lookup → rl-architect lean **EXCLUDE**, it's a test hook not a user financing knob) and `hurdle_rate_override` (the P(IRR<hurdle) threshold → rl-architect lean **EXCLUDE/common**, since apples-to-apples ranking (D42) wants ONE hurdle across compared configs). Any key not in the final set is **REJECTED** with: *"market-rate is common-scenario, not per-config; set it on `ComparisonSession.common_finance`."*
