...
- none — [B1] resolved by [D3]–[D10] (PR #TBD); the telemetry shared contract `contracts/shared/telemetry_schema.md` remains DRAFT pending APPROVE from BOTH backend-reviewer and frontend-reviewer (LOCKED entry to follow). [B2] resolved by [D11] (PR #TBD): parity reference is a from-scratch NumPy implementation under `reference/`, not vendored legacy.
