...
- **Device/unlimited:** **excellent and underappreciated** — the entire multi-year array is *tiny* (≈ 30 yr × 8760 h × ~4 vars × 4 B ≈ **4 MB**) and lives on-device permanently; each env samples a random sequence of block start-indices and gathers them via vmapped `lax.dynamic_slice`. Pure index math, jittable, zero host I/O, and **combinatorially unlimited** (the number of distinct stitched years vastly exceeds any training run). It reuses §12's data pipeline almost verbatim (just multi-year). ✓✓
...
- **Device/unlimited:** **moderate** — a sequential categorical regime draw (`lax.scan` with a per-step `categorical`) plus parametric within-regime sampling; jittable and vmappable, heavier than (a)/(b) but feasible; genuinely unlimited. ◐→✓
...
3. **Block granularity prior:** day-aligned blocks with seasonal stratification as the v1 default (vs week blocks — longer extremes but less variety)?
