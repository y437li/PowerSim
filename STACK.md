...
| **Frontend shell** | **React + Vite + TypeScript** | App shell, routing, websocket/REST clients. `npm run build` uses `tsc -p tsconfig.app.json` (src-only typecheck) so Node-flavoured build-time code in `scripts/` and test-only types in `tests/` are excluded from the production compile; Vitest/esbuild typechecks the full `tests/` tree independently | REBUILD_SPEC §6; charters; PR #62 |
