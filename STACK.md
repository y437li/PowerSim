...
| **Finance engine** | Pure **Python + NumPy** (`numpy` ≥1.24; `scipy` for IRR root-finding). No JAX — the engine is off-wire and pure-Python for maximum simplicity. Sub-modules: `cash_flow`, `metrics`, `price_path`, `distributions`, `discount`, `sensitivity`, `econ_params`, `engine` (facade). `tests/finance/` under `pytest`. | `src/energy_go/finance/`; all 7 sub-modules + facade are pure functions (no I/O). `scipy.optimize.brentq` for IRR; `np.quantile(..., method='lower'/'higher')` LOCKED as sole percentile estimator. `np.random.default_rng(seed)` for bootstrap CI. | D39 (PR #108); finance_engine contract |
| **Finance engine** (`energy_go.finance`) | Pure **Python + NumPy** (no JAX, no scipy); self-contained Brent's-method IRR solver; bootstrap CI via `np.random.default_rng` | 8 sub-modules + `engine.py` facade; pure function (no I/O); `tests/finance/` area | §13 / D39; PR #111 (task #6) |
