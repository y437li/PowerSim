...
| **Training** | **SAC** via **`sbx-rl`** (SB3-in-JAX; PyPI package `sbx-rl` — NOT `sbx` which is an unrelated flashcard app) or a custom pure-JAX loop; `optax`, `flax`; `flashbax` replay buffer; `jax.vmap` over ≥4096 envs end-to-end on device | Running-stat normalisation (`RunningStats` NamedTuple) saved with checkpoint (§5, §7); optional deps: `pyproject.toml [training]` | REBUILD_SPEC §5/§7; task #11 (PyPI identity fix, this PR) |
