...
`SiteScene.tsx` is the top-level R3F entry point: it mounts the canvas into a provided `containerEl`, renders 146 Gansu wind turbines as a single `InstancedMesh` (one draw call), and exposes a data-bridge `div` tree for testing. `SceneContent.tsx` places lights and GLB instances inside the R3F canvas and drives animation on each frame. All GLB paths are resolved through `registry.ts` (`resolveAsset`) — asset paths are never hardcoded in scene code; they are looked up from the LOCKED `assets/3d/registry.json` v1.0.0.
